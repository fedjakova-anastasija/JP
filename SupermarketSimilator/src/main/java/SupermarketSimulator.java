...
class SupermarketSimulator {
    public static void main(String[] args) {
        ...
        for (String element : productTypes) {
            if (productTypes.indexOf(element) == 6) {
                alcoholType = true;
            }
        ...
        supermarket.addCustomer(customerTypes.get(random.nextInt(2)), paymentMethods.get(random.nextInt(1)));
        ...
        while (isSuperMarketNotEmpty(supermarket)) {
            if (random.nextInt(supermarket.getCustomersCount()) > 0 || supermarket.getCustomersCount() > 3) {
